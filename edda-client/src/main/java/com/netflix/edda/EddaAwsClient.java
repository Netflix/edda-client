...
abstract public class EddaAwsClient {
  ...
  public EddaAwsClient(AwsConfiguration config, RequestHandler2 requestHandler) {
    this.config = config;
  ...
  protected EddaResponse doGet(final EddaRequest<?> request) {
    EddaResponse response = RxHttp.get(request.getUrl())
  ...
  static class EddaResponse {
    ...
    HttpResponse toHttpResponse(Request<?> request) {
      URI fullUri = URI.create(request.getEndpoint().toString() + request.getResourcePath()).normalize();
      HttpResponse response = new HttpResponse(request, new HttpGet(fullUri));
    ...
    <T> Response<T> toResponse(Request<?> request, T awsResponse) {
      return new Response<>(awsResponse, toHttpResponse(request));
    }
  }

  protected RuntimeException notifyException(Request<?> request, EddaResponse eddaResponse) {
  ...
  protected <T extends AmazonWebServiceRequest> EddaRequest<T> buildRequest(T originalRequest, String relativeUri) {
    EddaRequest<T> req = new EddaRequest<>(originalRequest, config.url(), relativeUri);
    if (requestHandler != null) {
  ...
  protected <T> T handleResponse(EddaResponse resp, Request<?> request, T awsResp) {
